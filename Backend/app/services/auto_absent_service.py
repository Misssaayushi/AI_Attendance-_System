...
def _insert_absent_records(db: Session, *, target_date: date, student_ids: list[int]) -> tuple[int, int, list[int]]:
    """
    ...
    absent_time = _resolve_absent_time()
...
def run_auto_absent_for_date(
    ...
    *,
    ...
    """
    Core workflow:
    Fetch all students -> fetch already-marked students -> identify unmarked ->
    insert absentees -> sync Excel.
    """
    ...
    try:
        student_ids = _fetch_student_ids(db)
        marked_ids = _fetch_marked_student_ids(db, target_date=run_date)
        unmarked_ids = identify_unmarked_student_ids(student_ids, marked_ids)

        auto_absent_marked, duplicate_skipped, inserted_student_ids = _insert_absent_records(
            db,
            target_date=run_date,
            student_ids=unmarked_ids,
        )
        excel_synced = 0
